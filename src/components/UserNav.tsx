  ...
  return (
    <div className="fixed top-6 right-6 z-50">
      <DropdownMenu>
        ...
        <DropdownMenuContent className="w-56 rounded-[1.5rem] p-2 shadow-2xl border-none" align="end">
          <DropdownMenuLabel className="font-black text-xs uppercase tracking-widest text-slate-400 px-4 py-3">
            Minha Conta
          </DropdownMenuLabel>
          <DropdownMenuSeparator className="bg-slate-100" />
          {isAdmin && (
            <DropdownMenuItem onClick={onAdmin} className="rounded-xl h-12 font-bold text-slate-700 focus:bg-primary/10 focus:text-primary cursor-pointer">
