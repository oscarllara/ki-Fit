    ...
    <div className="fixed top-6 right-6 z-50">
      <DropdownMenu>
        ...
        <DropdownMenuContent className="w-56 rounded-[1.5rem] p-2 shadow-2xl border border-white/10 bg-slate-900" align="end">
          ...
          {isAdmin && (
            <DropdownMenuItem onClick={onAdmin} className="rounded-xl h-12 font-bold text-white focus:bg-primary focus:text-slate-950 cursor-pointer">
              <Shield className="mr-2 h-4 w-4" />
              Painel Gestor
            </DropdownMenuItem>
          )}
          <DropdownMenuItem onClick={onProfileUpdate} className="rounded-xl h-12 font-bold text-white focus:bg-primary focus:text-slate-950 cursor-pointer">
            <User className="mr-2 h-4 w-4" />
            Meu Perfil
          </DropdownMenuItem>
          <DropdownMenuSeparator className="bg-white/5" />
          <DropdownMenuItem onClick={onLogout} className="rounded-xl h-12 font-bold text-destructive focus:bg-destructive/10 focus:text-destructive cursor-pointer">
